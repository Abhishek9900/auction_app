...
class SellerProfileForm(forms.ModelForm):
    ...
    def save(self,user, commit=True):
        seller = super(SellerProfileForm, self).save(commit=False)
        seller.user = user
        seller.save()
        return seller
...
class EditProfileForm(forms.ModelForm):
    ...
    def save(self,user, commit=True):
        seller = super(EditProfileForm, self).save(commit=False)
        seller.user = user
        seller.save()
        return seller
...
class ItemForm(forms.ModelForm):
    class Meta:
        model = Item
        fields = ['title', 'description', 'condition', 'category', 'status', 'image']
...
class AuctionEventForm(forms.ModelForm):
    ...
    def clean_start_time(self):
        ...
        if cleaned_start_time < utc.localize(datetime.now()):
    ...
    def clean_end_time(self):
        ...
        if cleaned_end_time < utc.localize(datetime.now()):
            raise ValidationError('Specified time occurs in the past.')
